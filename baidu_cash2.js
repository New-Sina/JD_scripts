...
if ($.isNode()) {
  ...
  BDCookie = process.env.BAIDU_COOKIE2.split('&');
  }
 else if (process.env.BAIDU_COOKIE2 && process.env.BAIDU_COOKIE2.indexOf('\n') > -1) {
  BDCookie = process.env.BAIDU_COOKIE2.split('\n');
  } else {
  BDCookie = process.env.BAIDU_COOKIE2.split()
  };
  if (process.env.BAIDU_CASH2 && process.env.BAIDU_CASH2.indexOf('&') > -1) {
  BDCASH = process.env.BAIDU_CASH2.split('&');
  }
 else if (process.env.BAIDU_CASH2 && process.env.BAIDU_CASH2.indexOf('\n') > -1) {
  BDCASH = process.env.BAIDU_CASH2.split('\n');
  } else {
  BDCASH = [process.env.BAIDU_CASH2]
  }
...
!(async() =>{
  ...
  for (let i = 0; i < CookieArr.length; i++) {
    if (CookieArr[i]) {
      ...
      await $.wait(1000);
      if ($.isNode()) {
        if (process.env.BAIDU_TASK2) {
         taskON = process.env.BAIDU_TASK2
       }
      } 
      if (taskON == "true") {
        $.desc = "";
